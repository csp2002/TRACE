...
def eval(image_paths, mask_paths, medsam_model, save_dir, annotation_path):
    ...
    for i in range(len(image_paths)):
        ...
        # print("img_1024_tensor:"+str(img_1024_tensor.shape))    #1,3,1024,1024
        # raise Exception
        ...
        info = annotation_dict.get(case_num)
        if isinstance(info, dict):
            ...
            #在读入的字典找到键为folder_name的值，如果没找到报错
        # if folder_name not in annotation_dict.keys():
        #     print('Error! Cannot find the annotation path of folder:', folder_name)
        # else:
        # annotation_path = annotation_dict[folder_name]
        # print('Use the one slice annotation:', ref_mask_path)
        ...
        annotation_prompt = cv2.resize(annotation_prompt, (mask.shape[0], mask.shape[1]), interpolation=cv2.INTER_NEAREST)
        ...
        if contours:
            ...
            # print("box_prompt2:",box_prompt.shape)
            
        else:
            print("Error! Cannot find the rectangle of the annotation:"+ ref_image_path)
        ...
        #     box_prompt = np.array([[x,y,x+w,y+h]])
        # else:
        #     print("cannot find the rectangle of:" + mask_path)
        ...
        with torch.no_grad():
            ...
            # print("image_embedding:",image_embedding[0].shape)

        medsam_seg = medsam_inference(medsam_model, image_embedding[0], box_1024, H, W)
        # print("res:"+str(medsam_seg.shape)+str(medsam_seg.max())+str(medsam_seg.min()))
...
if __name__ == "__main__":
    ...
    annotation_path = os.path.join('./2D_data',args.data, 'test','annotation_dict_middle.json')
