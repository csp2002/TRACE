...
# ================== Additional metrics for simulation_v4 ==================

def _binary(arr: np.ndarray) -> np.ndarray:
    """Ensure 2D binary numpy mask (bool)."""
...
def _boundary(mask_bool: np.ndarray) -> np.ndarray:
    """1-pixel-wide boundary of a binary mask (internal boundary)."""
    if mask_bool.sum() == 0:
...
def sdsc_2px(pred: np.ndarray, gt: np.ndarray, tau_px: float = 2.0) -> float:
    """Surface DSC (Nikolov et al. / MONAI) at pixel tolerance tau_px.
    sDSC = (|{pred boundary within tau of gt boundary}| + |{gt boundary within tau of pred boundary}|)
           / (|pred boundary| + |gt boundary|)
    """
    p = _binary(pred); g = _binary(gt)
    ...
    if pb.sum() == 0 or gb.sum() == 0:
        return 0.0
    # distance from every pixel to the nearest pixel on gb (resp. pb)
...
def fn_rate(pred: np.ndarray, gt: np.ndarray, eps: float = 1e-6) -> float:
    """FN rate = FN / (FN + TP) = 1 - recall. Range [0,1]; lower is better."""
    p = _binary(pred); g = _binary(gt)
    tp = float((p & g).sum())
    fn = float((~p & g).sum())
...
def fp_rate(pred: np.ndarray, gt: np.ndarray, eps: float = 1e-6) -> float:
    """FP rate = FP / (FP + TP) = 1 - precision (false discovery rate).
    Range [0,1]; lower is better. Defined as 0 when pred is empty."""
    p = _binary(pred); g = _binary(gt)
    if p.sum() == 0:
        return 0.0
    tp = float((p & g).sum())
...
def hd95(pred: np.ndarray, gt: np.ndarray) -> float:
    """95-percentile symmetric Hausdorff distance in pixels.
    If either mask is empty, returns np.nan."""
    p = _binary(pred); g = _binary(gt)
    pb = _boundary(p); gb = _boundary(g)
    if pb.sum() == 0 or gb.sum() == 0:
        return float("nan")
    ...
    hd95_pg = float(np.percentile(d_p_to_g, 95))
    hd95_gp = float(np.percentile(d_g_to_p, 95))
    return max(hd95_pg, hd95_gp)


# Metric registry: (callable(pred_np, gt_np) -> float, higher_is_better)
METRIC_FNS = {
    "dice":    (dice_np,  True),
    "sdsc":    (sdsc_2px, True),
    "fn_rate": (fn_rate,  False),
    "fp_rate": (fp_rate,  False),
    "hd95":    (hd95,     False),
}


def compute_metric(pred_np: np.ndarray, gt_np: np.ndarray, metric_name: str) -> float:
    if metric_name not in METRIC_FNS:
        raise ValueError(f"Unknown metric '{metric_name}'. Choices: {list(METRIC_FNS.keys())}")
    fn, _ = METRIC_FNS[metric_name]
    return float(fn(pred_np, gt_np))


def is_accepted(metric_value: float, threshold: float, metric_name: str) -> bool:
    """Accept iff the metric satisfies the (threshold, direction) rule.
    Higher-is-better: accept if metric > threshold.
    Lower-is-better:  accept if metric < threshold.
    NaN metric (e.g., empty masks for HD95) -> not accepted (force manual edit)."""
    if np.isnan(metric_value):
        return False
    _, higher_is_better = METRIC_FNS[metric_name]
    return (metric_value > threshold) if higher_is_better else (metric_value < threshold)
...
class DoctorSimTester:
    ...
    def __init__(
        ...
        metric: str = "dice",   # 'dice', 'sdsc', 'fn_rate', 'fp_rate', 'hd95'
    ):
        self.root = data_root
        ...
        assert self.metric in METRIC_FNS, f"metric must be one of {list(METRIC_FNS.keys())}, got {metric}"
    ...
    def _process_volume(self, pid: str) -> Dict:
        ...
        if self.ai_mode == "A":
            ...
            center_metric = compute_metric(center_pred, center_gt, self.metric)
            first_manual = int(not is_accepted(center_metric, self.th, self.metric))
    ...
    def _infer_slice(
        ...
        if self.model_type == "traditional":
            ...
            # Compute accept/reject metric at ORIGINAL resolution
            metric_val = compute_metric(pred_np, gt_np, self.metric)
        ...
        accepted = is_accepted(metric_val, self.th, self.metric)
        final_np = gt_np

        final_masks[idx] = final_np
        self._save_mask(final_np, pid, sl_name)

        return metric_val, accepted
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Accept/reject metric: dice, sdsc (2px), fn_rate, fp_rate, hd95. "
             "For 'higher-is-better' metrics (dice, sdsc) a slice is accepted iff metric > threshold; "
             "for 'lower-is-better' metrics (fn_rate, fp_rate, hd95) accepted iff metric < threshold.",
    )
    ...
    plt.xlabel(f"Threshold ({args.metric})")
