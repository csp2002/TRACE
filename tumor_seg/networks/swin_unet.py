...
class SwinUnet_ours(nn.Module):  
    def __init__(self, config,img_size=224, num_classes=2, refine_iters = 3, in_ch=1,
            detach_between_iters = True):
        super(SwinUnet_ours, self).__init__()
        self.swin_unet = SwinUnet(config, img_size=img_size, num_classes=num_classes)
    ...
    def forward(self, x,  x_ref, ref_gt):  #x: bs,1,224,224
        ...
        logits = self.swin_unet(x)  #bs,2,224,224
        logits_ref = self.swin_unet(x_ref)  #bs,2,224,224
